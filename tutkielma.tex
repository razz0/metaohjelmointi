...
Käännösaikaista metaohjelmointia ei ole suoraan tuettu Pythonissa. Mallien (template) käyttö Python-koodin luomiseen on mahdollista ajamalla mallia käsittelevä ohjelma ja tämän jälkeen kääntää mallin pohjalta luotu koodi. Tähän voidaan käyttää standardikirjastosta löytyvää yksinkertaista, mutta laajennettavaa \verb|string|-moduulin \verb|Template|-luokkaa tai jotakin Pythonin lukuisista mallimoottoreista (template engine) \cite{Templating}.
